// app/principal/overwork/page.tsx
"use client";
...
export default function PrincipalOverworkPage() {
  ...
  const fetchEntries = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/principal/overwork");
      const data = await response.json();
  ...
  const handleApprove = async (entryId: string) => {
    ...
    try {
      const response = await fetch(`/api/principal/overwork/${entryId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async (entryId: string) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/principal/overwork/${entryId}/reject`, {
        method: "POST",
      });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve overwork entries from HODs and Registrars
        </p>
        {config && (
