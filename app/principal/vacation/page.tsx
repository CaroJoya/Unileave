// app/principal/vacation/page.tsx
"use client";
...
export default function PrincipalVacationPage() {
  ...
  const fetchRequests = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/principal/vacation");
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`/api/principal/vacation/${requestId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await fetch(`/api/principal/vacation/${rejectModal.requestId}/reject`, {
        method: "POST",
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve vacation requests from HODs and Registrars
        </p>
      </div>
