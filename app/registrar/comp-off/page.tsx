// app/registrar/comp-off/page.tsx
"use client";
...
export default function RegistrarCompOffPage() {
  ...
  const fetchCredits = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/registrar/comp-off");
  ...
  const handleApprove = async (creditId: string) => {
    ...
    try {
      const response = await fetch(`/api/registrar/comp-off/${creditId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async (creditId: string) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/registrar/comp-off/${creditId}/reject`, {
        method: "POST",
      });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve comp-off requests from office staff
        </p>
      </div>
