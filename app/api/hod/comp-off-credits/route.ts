// app/api/hod/comp-off-credits/route.ts - NEW FILE
...
export async function GET() {
  try {
    ...
    const hodData = hodSnapshot.val() as UserData | null;
    ...
    for (const [id, credit] of Object.entries(allCredits)) {
      if (credit.status === 'pending_approval' && departmentUserIds.includes(credit.userId)) {
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'approve') {
      result = await approveCompOffCredit(creditId, hodId, hodData.name, remark);
      notificationMessage = `Your comp-off credit request for ${credit.creditedDays} day(s) has been approved by HOD ${hodData.name}.`;
      notificationType = NotificationType.COMPOFF_APPROVED;
    } else {
      result = await rejectCompOffCredit(creditId, hodId, hodData.name, remark);
      notificationMessage = `Your comp-off credit request for ${credit.creditedDays} day(s) has been rejected by HOD ${hodData.name}. Reason: ${remark}`;
