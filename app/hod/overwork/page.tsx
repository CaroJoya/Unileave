...
export default function HODOverworkPage() {
  ...
  const fetchEntries = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/hod/overwork");
      const data = await response.json();
  ...
  // Load data - fetchConfig and fetchEntries both call setState
  // This is fine as they are async operations, not synchronous setState
  useEffect(() => {
  ...
  const handleApprove = async (entryId: string) => {
    ...
    try {
      const response = await fetch(`/api/hod/overwork/${entryId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async (entryId: string) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/hod/overwork/${entryId}/reject`, {
        method: "POST",
      });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve overwork entries from your department
        </p>
        {config && (
        ...
        <CardContent className="pt-6">
          {pendingEntries.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              No pending overwork requests from your department.
            </div>
          ) : (
