...
export default function HODVacationPage() {
  ...
  const fetchRequests = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/hod/vacation");
...
useEffect(() => {
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`/api/hod/vacation/${requestId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await fetch(`/api/hod/vacation/${rejectModal.requestId}/reject`, {
        method: "POST",
        ...
        <CardContent className="pt-6">
          {pendingRequests.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              No pending vacation requests from your department.
            </div>
          ) : (
