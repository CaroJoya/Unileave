...
export default function HODCompOffPage() {
  ...
  const fetchCredits = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/hod/comp-off");
...
useEffect(() => {
  ...
  const handleApprove = async (creditId: string) => {
    ...
    try {
      const response = await fetch(`/api/hod/comp-off/${creditId}/approve`, {
        method: "POST",
      });
  ...
  const handleReject = async (creditId: string) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/hod/comp-off/${creditId}/reject`, {
        method: "POST",
      });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve comp-off requests from your department
        </p>
      </div>
        ...
        <CardContent className="pt-6">
          {pendingCredits.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              No pending comp-off requests from your department.
            </div>
          ) : (
